...
- GDD·체크리스트·`v1_assumptions` 등에 **미정**이거나 **실무적으로 갈래가 나는 스펙**은, 코드·문서에 반영하기 전에 **반드시 사용자에게 확인**받는다. (추측으로 잠정 구현만 하고 넘어가지 않는다. 잠정안이면 그 사실을 명시한다.)
...
- **폰 기준 세로 UI + 태블릿에서 가로 여백·배경만 확장**하는 패턴은 `docs/responsive-phone-frame-layout.md`를 따른다.
