...
| **GameView** | `GameWidget`에 `BingoCardGame` 마운트, Countdown/PauseMenu/Clear 오버레이 빌더 |
...
```
lib/game/
├── bingo_card_game.dart      # FlameGame 메인 (규칙, 상태, 이벤트)
└── components/
...
| 클래스 | 역할 |
|--------|------|
| **BingoCardGame** | 게임 상태(currentNumber, elapsedTime), 셔플·그리드 생성, 탭 이벤트, 라이프사이클(백그라운드 일시정지) |
| **CubeButton** | 숫자/알파벳 표시, 정답 시 회전·페이드아웃, 오답 시 흔들림, 힌트 시 깜빡임 |
| **GameHud** | 타임 패널, 힌트 말풍선, 일시정지 버튼, 베스트 스코어 표시 |

### 진행 상황

Rummi Poker Grid 전환 작업은 `docs/rummi_poker_grid_execution_checklist.md` 체크리스트를 따른다.
...
| Web | `flutter build web --release --base-href "/binggocard/"` |

Web 빌드 시 용량 줄이기: `--tree-shake-icons --no-source-maps` 옵션 추가.  
정적 호스팅 배포 시 `binggocard` 폴더 생성 후 `build/web/*` 복사.  
→ 상세: [`docs/web_build.md`](docs/web_build.md)

## 문서

- [`docs/rummi_poker_grid_execution_checklist.md`](docs/rummi_poker_grid_execution_checklist.md) - Rummi Poker Grid 실행 체크리스트
- [`docs/rummi_poker_grid_gdd.md`](docs/rummi_poker_grid_gdd.md) - 기획 GDD
- [`docs/rummi_poker_grid_migration_plan.md`](docs/rummi_poker_grid_migration_plan.md) - 마이그레이션·폴더 플랜
