...
- Market 후보 슬롯 증가 효과는 화면이 감당하는 후보 슬롯 상한과 함께 관리한다. 현재 Jester/Item 후보 슬롯은 4개를 상한으로 보고, `shop_lens`, `boss_trophy`처럼 후보 슬롯을 늘리는 효과는 상한 도달 시 실패/미소모 처리하며 일부만 적용 가능하면 실제 증가량만 이벤트에 남긴다.
- `shop_lens`처럼 Item 후보 수를 늘리는 효과는 후보 영역에 `렌즈 +1` 같은 출처 배지를 보여, 늘어난 후보가 아이템 효과 때문임을 즉시 알 수 있게 한다.
