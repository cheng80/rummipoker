...
- 풀런봇 판단을 CLI 시뮬레이션에 옮긴 bot policy는 `full_run_policy_v1`로 부른다. `contest_policy_v1`는 과거 산출물 경로/로그 이력에서만 보이는 legacy 이름으로 취급하고 새 실행, 문서, 응답에서는 쓰지 않는다.
...
- `emergency_draw`는 기본 드로우 보조가 아니라 덱 소모 없는 비상 생성 아이템이다. 손패가 비어 있고 전투가 아직 진행 중이면 덱이 0이어도 성공해야 하며, 생성 후보에서 강화/봉인/에디션 특수 타일을 배제하지 않는다. 단, 사용하지 않은 비상 드로우나 자원 보충 아이템이 있다는 이유로 덱 0/손패 0 같은 게임오버 판정을 미루지 않는다. 유저가 게임오버 상태가 오기 전에 직접 사용해야 하는 전략 리스크로 둔다.
...
- **상점 리롤 비용 분리**: Jester/Tile/Q-Slot/Passive/Tool/Gear 리롤은 각 offer lane별로 독립된 비용을 가진다. 단, S1 기본 첫 Market의 `첫 리롤 무료` 보상은 lane별 독립 상태가 아니라 Market 전체 1회 혜택이므로 한 lane에서 사용하면 다른 lane에는 남지 않는다. 아이템/패시브의 리롤 할인은 해당 효과 정책에 따라 별도 적용한다.
...
- 카드 하단 타입 텍스트는 실제 카드 face에서 제거한다. 카드 이름이 두 줄까지 필요할 수 있으므로 타입 구분은 우상단 type badge(`J/Q/P/G/T`)와 정보 overlay/tag에서만 제공한다.
...
- 전투 중 사용하는 신규 특수/Ritual 카드는 현재 전투의 배치, 확정, 점수, 자원, 드로우에 즉시 읽히는 변화를 줘야 한다. 다음 전투 덱 예약처럼 현재 전투에 도움 안 되는 효과는 일반 마켓 pool에 넣지 않고, 재설계 전까지 debug fixture/문서 검토 자산으로만 둔다.
- 운명/Fate 카드류는 선택한 보드 선을 실제 5칸 타일 세트로 바꿔 현재 전투 족보를 즉시 만들거나 바꾸는 효과만 normal market에 노출한다. 덱 예약형 복사, 제거 후보, 각인, 성장 의식처럼 효과가 다음 전투나 간접 상태에 머무르는 카드는 재설계 전까지 `ritual` 보류군으로 둔다.
- 로얄 플러시/로얄 스티플은 유저 노출 문구에서 카드 문자 표기를 쓰지 않는다. Rummi Poker 숫자 체계에서는 같은 색 `1,10,11,12,13`만 로얄로 판정하고, 같은 색 `9,10,11,12,13`은 일반 스티플로 남긴다.
- 로얄 플러시/로얄 스티플을 유저에게 숫자 순서로 설명할 때는 높은 진행감을 위해 `10-11-12-13-1` 순서로 표기한다. 내부 정렬/판정은 `1,10,11,12,13`을 유지한다.
- hidden 족보에는 플러시 하우스와 플러시 파이브를 포함한다. 플러시 하우스는 같은 색의 풀하우스, 플러시 파이브는 같은 색 같은 숫자 5장으로 판정하며, 일반 풀하우스/파이브카드보다 우선한다.
- 운명/Fate 카드의 선 선택은 큰 중앙 dialog로 보드를 가리지 않는다. 실제 전투 보드 위의 선 overlay를 직접 탭해 선택하고, 하단 확인 패널에서 기준 숫자/색상과 변환 결과를 읽은 뒤 별도 확인으로 적용한다. 적용 후에는 선택 선 flash 등으로 보드 변화가 눈에 띄어야 한다.
- 운명/Fate 카드 선 선택 모드의 입력 차단은 버튼별 `IgnorePointer`만으로 처리하지 않는다. 기존 `GameInputBarrier.modal()`을 전체 화면에 한 장 깔고, 그 위에 보드 선 선택 hit layer와 확인 패널을 올리며, 일시정지/옵션/정보 overlay는 그보다 높은 계층에 둔다.
- 카드 face UI를 검증하는 위젯 테스트는 카드 이름/타입 텍스트를 탭 대상으로 삼지 않는다. 카드 이름은 두 줄 처리, 축약, 이미지 중심 렌더링 정책에 따라 바뀔 수 있으므로 slot key, content id key, action button key 같은 안정된 테스트 key를 우선한다.
- Ritual/Fate의 선 또는 타일 선택은 실제 보드 위 후보를 직접 탭하는 방식으로 통일한다. 후보는 파란 둥근 사각형, 현재 선택은 주황 둥근 사각형으로 표시하고, 탭 즉시 적용하지 않고 상단 확인 패널의 확인/변환 버튼을 거쳐 발동한다.
- Ritual 효과가 보드 줄을 제거하거나 덱/골드를 바꾸면 결과를 숫자만 바꾸지 않는다. 제거되는 줄, 골드 HUD로 가는 골드, 덱 맨 위에 추가되는 타일을 각각 화면에서 읽히는 flight/피드백으로 보여주고, 다음 드로우에 바로 영향을 주는 효과는 현재 session deck에도 즉시 반영한다.
- Ritual 덱 보충 연출은 어떤 타일이 덱에 들어가는지 읽을 수 있어야 한다. 생성 타일을 출발 지점에서 잠깐 고정 표시한 뒤 덱으로 이동시키고, 화면 duration과 overlay state 해제 duration을 같은 전용 timing 상수로 관리한다.
- Ritual 골드 획득 연출은 단일 배지 이동으로 끝내지 않는다. 여러 동전이 우측 상단 Gold HUD까지 순차적으로 날아가는 느낌을 주고, 목적지는 Gold 표시 영역에 맞춘다.
- 카드, 타일, 점수, 골드처럼 한 위치에서 다른 위치로 이동하는 flight/move 연출은 개별 위젯에서 직접 `Offset.lerp`/`Alignment.lerp`와 임의 curve를 섞지 않는다. 공용 motion helper를 통해 easeOut 계열 이동 progress를 적용하고, hold/scale 같은 특수 박자만 해당 연출에서 별도로 조정한다.
- Ritual 덱 복사/메아리 카드가 보드 타일을 대상으로 삼을 때는 해당 줄의 `scoringTiles`만 후보로 쓴다. 선택 가능한 후보가 없는 줄은 선 후보에서 제외하고, 추가된 타일은 `addedDeckTiles` 기록뿐 아니라 현재 session deck 맨 위에도 넣어 다음 draw와 덱 flight에서 바로 체감되게 한다.
- Ritual/Fate 선택 안내에서 후보 강조색을 설명할 때 `파란 타일 후보`처럼 타일 색상과 헷갈리는 표현을 쓰지 않는다. 후보 UI 색은 `파란 테두리`, `강조된 후보`처럼 시각 표시로 부르고, `각인 복사`처럼 modifier가 조건인 카드의 복사 결과는 실제 deck/session/flight 표시 모두 seal/enhancement를 보존해야 한다.
- Ritual/Fate 카드 ID와 타일 modifier 저장값은 같은 문자열을 재사용하지 않는다. 운명 변환 카드는 `fate_*` 또는 효과가 드러나는 카드 ID를 쓰고, 타일 seal은 `fracture_seal`, `anchor_seal`, `echo_seal`처럼 별도 persistence value로 유지한다. 과거 운명 카드 ID였던 `risk_seal`, `anchor_seal`, `echo_seal`, `gold_seal_stamp`, `growth_seal`, `line_seal_stamp`는 item legacy alias로만 읽는다.
...
- 유저용 북마크는 이어하기 저장과 별도 슬롯으로 보관하되, 북마크를 불러오면 현재 이어하기 저장을 해당 북마크 상태로 덮어쓴다는 고지를 반드시 표시한다.
- 북마크 슬롯은 3개 고정이며 삭제 기능 없이 빈 슬롯은 즉시 저장, 기존 슬롯은 덮어쓰기 확인 후 저장한다. 라벨은 `S1 · 표준 · 하이 · SCOUT`처럼 Station, 난이도, modifier, 현재 전투 위치가 읽히게 만든다.
- 북마크 불러오기는 메인 화면뿐 아니라 게임 중 옵션/Market 옵션에서도 접근 가능해야 한다. 게임 중 불러오기는 현재 이어하기를 덮어쓴다는 확인을 거친 뒤 현재 화면 상태도 선택한 북마크 runtime으로 전환한다.
- `현재 전투 재시작`은 `stakeStartSnapshot` 기준으로 복원하고, `현재 Station 재시작`은 Station 시작 snapshot 기준으로 복원한다. 무한 구간의 Station 재시작은 진행을 크게 되돌릴 수 있으므로 별도 경고 확인을 거친다.
- 게임 오버는 판정 직후 결과창을 바로 띄우지 않는다. 화면 입력을 막는 빨간 페이드 연출을 먼저 보여주고 `TimeUp.wav`를 재생한 뒤, 연출 위에 게임 오버 dialog를 띄운다.
- 게임 용어 설명에서 배수/비율/칩 같은 계산 개념은 `강한`, `약한`처럼 수학적으로 모호한 강약 표현을 붙이지 않는다. 적용 순서와 실제 효과를 기준으로 설명한다.
- 첫 리롤 무료 혜택은 S1 기본 첫 Market(`stageIndex == 1 && currentStationBlindTierIndex == 0`)에서만 주어지는 Market 전체 1회 혜택으로 유지한다. 같은 S1 안의 다음 Market이나 S2 이후 Market에서는 다시 생기면 안 된다. 한 lane에서 사용하면 다른 lane에는 남지 않고, 쓰지 않은 채 다음 Market으로 넘어가도 사라진다. `리롤 칩`은 이와 별개로 다음 리롤 1회 비용을 1G만 줄이고 사용 시 소비된다.
- 첫 리롤 무료 같은 transient Market 혜택은 `openShop()`에서 새로 세팅할 때뿐 아니라 effective cost 계산과 save restore 경로에서도 eligibility를 다시 확인한다. 오래된 save에 `firstRerollDiscount`가 남아 있어도 S1 첫 Market이 아니면 UI에 `리롤 5→0`으로 보이면 안 된다.
...
- gstack-codex 설정은 repo-local skill docs만 유지한다. `.agents/skills/gstack/`에는 `SKILL.md`만 두고, upstream gstack runtime인 `bin/`, `browse/`, `qa/`, `review/`, `design/`, `VERSION`, `CHANGELOG.md`, `ETHOS.md`를 복사하지 않는다. 갱신은 프로젝트 루트에서 `npx --yes gstack-codex@latest init --project`로 수행한다.
