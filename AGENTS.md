...
- 모든 문서는 처음 읽는 사람도 전체 흐름을 쉽게 따라갈 수 있도록 내용의 순서와 문장을 풀어 쓴다. 작성이 끝나면 사용 가능한 윤문 스킬로 한 번 다듬는다. 윤문 스킬이 없으면 이 단계는 건너뛰고 최종 보고에 윤문하지 못했음을 밝힌다.
- 작업 전 기존 코드, 문서, 테스트와 구조를 확인하고 기존 패턴·도구를 재사용한다.
- 가장 단순하고 작은 올바른 변경을 선택하며 사용자 변경을 덮어쓰거나 되돌리지 않는다.
- 현재 제공되지 않는 외부 에이전트 도구의 명령·산출물(`.omo`, `.gstack`, `.cursor`, `.serena` 등)을 계획, 상태 또는 완료 근거로 사용하지 않는다. 필요한 계약과 검증 결과는 담당 `docs/`, `.github/instructions/`, 테스트로 이관하고 폐기된 도구 경로를 참조하지 않는다.
