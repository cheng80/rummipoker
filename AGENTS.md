...
- 첫 리롤 무료 혜택은 S1 기본 첫 Market 진입에서만 주어지는 Market 전체 1회 혜택으로 유지한다. 한 lane에서 사용하면 다른 lane에는 남지 않고, 쓰지 않은 채 다음 Market으로 넘어가도 사라진다. `리롤 칩`은 이와 별개로 다음 리롤 1회 비용을 1G만 줄이고 사용 시 소비된다.
