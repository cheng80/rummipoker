...
- 결론: sequence/path 모델은 후보 선별 보조 신호로 유지한다. 단, 런타임 자동 적용 근거는 아니다.
...
- random split MAE: 0.0509
- random split RMSE: 0.0905
- random split R2: 0.9014
- source split MAE: 0.0582
- source split RMSE: 0.1120
- source split R2: 0.8408

해석:

- post-run result를 볼 수 없으므로 이전 outcome-summary scaffold보다 점수가 약한 것이 자연스럽다.
- RMSE `0.0905`는 random split 기준이고, source split RMSE `0.1120`이 더 보수적인 값이다.
