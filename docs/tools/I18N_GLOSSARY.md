...
영어 낱말이 한자나 가나 옆에 올 때는 `Gold 不足`, `Station 目标`, `Market 候補`처럼 한 칸을 띄운다. 용어집의 `Gold 不足` 행을 따른 것이다.

**사용자 검수가 필요한 항목.** 위 표에서 근거가 `다수 표기`인 줄은 용어집에 결정이 없어서 지금 더 많이 쓰인 쪽으로 맞춘 것이다. 타일의 중국어, 전투의 일본어, 그리고 `Gold`·`Station`·`Market`을 계속 영어로 둘지가 여기에 해당한다. 영어 낱말 옆의 띄어쓰기 규칙도 함께 봐 주기 바란다.
...
- 일본어·중국어에서 `Chips`, `Gold`, `Station`, `Jester`처럼 영어를 그대로 둔 용어를 계속 영어로 둘지.
