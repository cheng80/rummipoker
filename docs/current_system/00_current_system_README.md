...
`CURRENT_LEVELING_ML_BASELINE.md`는 과거 경로 호환용 deprecated 문서다. 현재 기준은 실제 머신러닝이 아니라 시뮬레이션과 휴리스틱 진단이다. 구 ML 산출물은 `docs/archive/leveling/legacy_ml_outputs_2026_05/`로 이동했으며, 새 ML 전환 상태는 `analysis/leveling/README.md`에서 fresh workspace 기준으로 확인한다.
