...
과거 `CURRENT_LEVELING_ML_BASELINE.md`는 `docs/archive/leveling/deprecated_2026_05/`로 이동했다. 현재 기준은 실제 머신러닝이 아니라 시뮬레이션과 휴리스틱 진단이다. 구 ML 산출물은 `docs/archive/leveling/legacy_ml_outputs_2026_05/`로 이동했으며, 새 ML 전환 상태는 `analysis/leveling/README.md`에서 fresh workspace 기준으로 확인한다.
...
- 현재 보스전 보드 칸 금지 패턴: `BOSS_BOARD_CELL_BLOCK_PATTERNS.md`
