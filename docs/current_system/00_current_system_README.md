...
레벨링/마켓 정책을 다룰 때는 `CURRENT_LEVELING_POLICY.md`, `CURRENT_LEVELING_SIMULATION_BASELINE.md`, `CURRENT_LEVELING_RUNTIME_SPEC.md`를 추가로 읽는다. 이 문서들은 과거 시뮬레이션 로그보다 우선한다.

현재 기준은 실제 머신러닝이 아니라 시뮬레이션과 휴리스틱 진단이다. 구 ML 산출물과 분석 workspace는 현재 판단 경로에서 제거됐으며, 새 ML 전환 상태 문서로 읽지 않는다.
