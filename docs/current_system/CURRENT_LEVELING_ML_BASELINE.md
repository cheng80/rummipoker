...
기존 `analysis/leveling/` feature table과 RandomForest 산출물은 active workspace에서 제거하고 `docs/archive/leveling/legacy_ml_outputs_2026_05/`로 이동했다. 해당 산출물은 실제 ML 전환 완료 증거가 아니며, target/boss/market/economy 후보를 추천하거나 런타임에 적용하는 근거로 쓰지 않는다.

새 `preoutcome` 산출물이 추가되어도 fresh 데이터와 supervised validation이 닫히기 전까지는 planned transition scaffold다. 실행 전 feature만 쓰는 baseline은 실제 ML 이행을 위한 준비 단계이지만, production ML이나 자동 밸런스 조정 완료 근거가 아니다.
