...
- 기존 S4 Scout에서 `4535 / 580`처럼 성장한 run이 목표를 약 7.8배 초과하는 사례가 확인되어, S1~S8 target을 로그형 상향 표로 교체했다.
- S1 Scout는 기존 `240`의 2배인 `480`으로 두고, 중후반으로 갈수록 더 큰 폭으로 벌어지게 했다.
- 같은 station 안에서는 `Scout < Clash < Boss` 압박을 명확히 두며, Boss는 단순 보너스 전투가 아니라 성장 체크포인트로 본다.
- 이 표는 런타임 적용값이다. 장기 clear rate와 경제 snowball 여부는 별도 sweep 전까지 open risk로 남긴다.
