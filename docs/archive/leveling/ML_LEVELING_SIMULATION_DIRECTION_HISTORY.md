...
v84 v72 exposure smoke 재확인:

- 목적:
  - `missing growth market exposure` 보정이 실제 route/market 성능을 과하게 올리지 않는지 빠르게 확인한다.
  - 이 결과는 판단용 smoke이며, 기존 장기 sweep을 대체하지 않는다.
- 파일:
  - `logs/sim/ml_sweep_v72_exposure_smoke_v84_r120_summary.json`
  - `logs/sim/ml_sweep_v72_exposure_smoke_v84_r120_report.md`
- 조건:
  - runs: 120
  - sequence: S1~S8 small/big/boss path
  - difficulty: `standard`
  - experiment: `base_score_curve_v2_boss_constraint_pool_v4_s1_soft_v2_late_guard_v1_s1_resource_weighted_boss_v3_late_boss_068`
  - loadout: `progression_route_balanced`, `progression_route_power`
  - market: `none`, `shop_slot_market_v9`
...
| loadout | market | path clear | avg total turn | avg clear path turn | 주요 stop |
|---|---|---:|---:|---:|---|
| balanced | none | 55.8% | 1361.6 | 1752.5 | board 27, draw 25 |
| balanced | v9 | 57.5% | 1319.7 | 1669.3 | board 38, draw 12 |
| power | none | 65.0% | 1336.8 | 1629.7 | board 25, draw 17 |
| power | v9 | 73.3% | 1377.9 | 1554.9 | board 21, draw 11 |

S1/S4/S5/S8 병목:

| loadout | market | S1 | S4 | S5 | S8 |
|---|---|---:|---:|---:|---:|
| balanced | none | 11 | 10 | 14 | 4 |
| balanced | v9 | 11 | 5 | 5 | 12 |
| power | none | 14 | 3 | 2 | 10 |
| power | v9 | 5 | 1 | 3 | 11 |

상위 failure:

| loadout | market | top failures |
|---|---|---|
| balanced | none | S5 boss 12, S4 big 5, S4 boss 5, S1 boss 5, S1 big 4 |
| balanced | v9 | S8 boss 9, S1 small 5, S2 small 5, S1 big 4, S5 boss 3 |
| power | none | S1 boss 7, S8 boss 7, S1 big 5, S8 big 3, S7 boss 3 |
| power | v9 | S8 boss 9, S2 boss 4, S1 boss 3, S5 boss 2, S8 big 2 |

v84 판정:

- v9 exposure는 과보정으로 보이지 않는다. power route에서 path clear가 65.0% -> 73.3%로 오르지만, full S1~S8 24전 path 기준으로는 아직 실패 여지가 충분하다.
- balanced route는 55.8% -> 57.5%로 소폭 개선이며, avg clear path turn도 줄었다.
- v9는 draw exhausted를 줄이는 대신 일부 조건에서 board lock 비중을 남긴다. 특히 balanced는 draw 25 -> 12로 줄고 board 27 -> 38로 늘어, 다음 병목은 slot/weight 상향보다 board pressure 후보 availability와 유저 선택 proxy 쪽에서 봐야 한다.
- S1/S4/S5는 v9에서 전반적으로 완화된다. 잔여 병목은 S8 boss 쪽으로 이동한다.
- 현재 기준에서 slot focus 확률을 낮출 필요는 없다. 너무 강한 직접 지급/고정 슬롯 효과는 보이지 않으며, 기존 원칙대로 후보 availability와 랜덤 슬롯 노출만 유지한다.

v85 S8 boss 축 분리:

- 목적:
  - v84에서 v9 적용 후 잔여 병목이 S8 boss 쪽으로 이동했으므로, S8 boss 단일 전투의 constraint별 실패 축을 확인한다.
  - 이 probe는 late route 누적 상태를 완전히 재현하지 않는 단일 battle smoke다. 적용 판단은 장기 sequence 결과와 함께 본다.
- 파일:
  - `logs/sim/s8_boss_axis_v85_r400.jsonl`
  - `logs/sim/s8_boss_axis_v85_r400_summary.json`
- 조건:
  - runs: 400
  - station: S8 boss only
  - difficulty: `standard`
  - experiment: `base_score_curve_v2_boss_constraint_pool_v4_s1_soft_v2_late_guard_v1_s1_resource_weighted_boss_v3_late_boss_068`
  - loadout: `s8_finale_build`
  - market: none

S8 boss constraint별 결과:

| constraint | runs | clear | avg turn | avg score ratio | stop |
|---|---:|---:|---:|---:|---|
| `all_score_dampener` | 100 | 95.0% | 92.8 | 1.05 | deck 5 |
| `color_dampener_cycle` | 9 | 55.6% | 105.8 | 1.02 | deck 4 |
| `confirm_count_tax_v2` | 68 | 76.5% | 102.2 | 1.00 | deck 15, board 1 |
| `face_tile_dampener` | 7 | 85.7% | 91.3 | 1.02 | deck 1 |
| `first_confirm_tax` | 43 | 100.0% | 83.8 | 1.06 | - |
| `line_kind_dampener_cycle` | 15 | 86.7% | 94.3 | 1.04 | deck 2 |
| `repeat_rank_pressure_v4` | 34 | 85.3% | 94.9 | 1.04 | deck 5 |
| `resource_squeeze` | 68 | 92.6% | 92.8 | 1.05 | deck 4, board 1 |
| `single_rank_pressure` | 14 | 85.7% | 95.9 | 1.04 | deck 2 |
| `target_spike_wall` | 42 | 78.6% | 99.5 | 0.99 | deck 7, board 2 |
...
- S8 boss 단일 전투는 전체 clear 87.8%로 “너무 쉬운” 상태가 아니다. v9가 route clear를 올려도 S8 boss 잔여 실패가 남는 것은 자연스럽다.
- 실패 대부분은 board lock이 아니라 deck exhausted다. 따라서 다음 조정 후보가 있다면 board slot/board discard 보정이 아니라, 후반 상점에서 덱/타일 형상 보정 후보와 점수 전환 후보가 등장 가능한지부터 본다.
- 약한 축은 `color_dampener_cycle`, `confirm_count_tax_v2`, `target_spike_wall`이다.
- `late_boss_068`을 더 낮추거나 slot focus를 낮출 근거는 아직 없다. 현재 수치는 S8 finale가 충분히 긴장감을 유지하는 쪽이다.
- 다음 분석은 v9 late offer가 덱/타일 형상 보정 후보와 점수 전환 후보를 충분히 노출하는지 확인한다. 직접 지급이나 자동 resource 증가는 적용 후보가 아니다.

v86 late offer exposure 확인:

- 목적:
  - v85에서 S8 boss 실패가 deck exhausted 중심으로 나타났으므로, `shop_slot_market_v9`가 후반에 덱/타일 형상 보정 후보와 점수 전환 후보를 얼마나 노출하는지 확인한다.
  - summary-only가 아니라 JSONL을 남겨 `market_shop_slots`, `market_purchase_events`, 실패 위치를 같이 본다.
- 파일:
  - `logs/sim/late_offer_exposure_v86_r200.jsonl`
  - `logs/sim/late_offer_exposure_v86_r200_summary.json`
- 조건:
  - runs: 200 per loadout
  - sequence: S1~S8 small/big/boss path
  - difficulty: `standard`
  - experiment: `base_score_curve_v2_boss_constraint_pool_v4_s1_soft_v2_late_guard_v1_s1_resource_weighted_boss_v3_late_boss_068`
  - market: `shop_slot_market_v9`
...
| loadout | runs | path clear | avg total turn | 주요 S8 failure |
|---|---:|---:|---:|---|
| balanced | 200 | 71.0% | 1421.9 | S8 boss draw 5, S8 boss board 1, S8 big draw 1 |
| power | 200 | 66.5% | 1313.0 | S8 boss draw 6, S8 big draw 2, S8 small board 2, S8 big board 2 |

Late row 결과:

| scope | rows | clear | deck exhausted | board locked | avg turn |
|---|---:|---:|---:|---:|---:|
| S6~S8 all | 2754 | 2712 | 19 | 23 | 66.6 |
| S8 all | 883 | 859 | 15 | 9 | 77.3 |
| S8 boss | 289 | 275 | 11 | 3 | 86.6 |
...
| purchase | count |
|---|---:|
| `rare_xmult_jester_proxy` | 244 |
| `planet_rank_level_proxy` | 38 |
| `uncommon_build_jester_proxy` | 6 |
| `legendary_bridge_proxy` | 1 |
...
| constraint | failed count |
|---|---:|
| `confirm_count_tax_v2` | 6 |
| `all_score_dampener` | 3 |
| `resource_squeeze` | 2 |
| `repeat_rank_pressure_v4` | 1 |
| `single_rank_pressure` | 1 |
| `target_spike_wall` | 1 |

v86 판정:

- v9 late는 점수 전환 후보를 충분히 노출한다. S8 boss에서 `rare_xmult`, `planet`, `uncommon build`가 각각 79~84% 수준으로 보인다.
- 반대로 마켓에서 구매해야 하는 덱/타일 형상 보정 후보의 노출은 낮다. S8 boss 기준 `tarot build pack` 16.3%, `build aware pack +5` 12.8%, `discard glove` 12.1%, `voucher resource` 5.9%, `tile pack +5` 3.5%다.
- 실패한 S8 boss run에서도 bot proxy는 `rare_xmult` 또는 `planet`만 구매했다. 즉 현재 proxy는 후반 실패 직전에도 점수 후보를 우선하고, 고갈 대응 후보를 거의 선택하지 않는다.
- 따라서 다음 적용 후보는 slot 수 증가나 직접 자원 지급이 아니다. 먼저 S7~S8 boss band에서 덱/타일 형상 보정 후보가 “등장 가능한 후보군”으로 더 안정적으로 포함되는지 확인해야 한다.
- `voucher_resource`는 시뮬 proxy 이름일 뿐 자동 resource 지급 후보가 아니다. 실제 적용 시에는 마켓에 등장할 수 있고 유저가 직접 구매해야 하는 후보로만 해석한다.
- 유저 선택 원칙은 유지한다. 후보는 마켓에만 뜨고, 슬롯이 없거나 돈이 부족하면 팔고 살지는 유저 몫이다. 시스템이 아이템/Jester/Pack/덱 타일/자원을 직접 지급하지 않는다.
- 코드 적용 후보는 `shop_slot_market_v9`의 late boss availability floor다. 단, 고정 슬롯 위치를 만들지 않고, 후보 위치는 stage/reroll/rng에 따라 흔들리게 유지한다.
