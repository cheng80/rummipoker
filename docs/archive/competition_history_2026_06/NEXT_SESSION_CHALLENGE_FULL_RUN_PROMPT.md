...
- 잠긴 슬롯 해금 룰은 2026-05-10 반영됐다. S2 Boss는 Quick Item 3번 슬롯, S4 Boss는 Passive 2번 슬롯, S6 Boss는 Jester 5번 슬롯을 해금하고 다음 Market 진입에서 1회 배너/슬롯 pulse가 떠야 한다.
...
4. 최신 build에서 정산 sheet가 PhoneFrame 안에 머무르고 노란 밑줄이 없는지, 홈 메뉴/도감/웹 BGM/공유 이미지 산물이 최신 기준인지 빠르게 눈검증한다.
5. `/game?fixture=slot_unlock_market`는 이미 눈검증 완료됐지만, 필요하면 자동 튜토리얼 없이 슬롯 해금 배너/자물쇠 해제/슬롯 pulse를 보조 확인한다. 이 fixture는 full-play evidence가 아니며, 실제 S2/S4/S6 Market 진입 검증은 full-run에서 다시 한다.
6. 공모전 풀런봇 policy code/test가 추가 덱, 보상 타일, 특수 족보, 족보 성장 점수, 슬롯 해금 상태를 실제 후보 평가/화면 판단에 반영하는지 확인한다.
7. 공모전 풀런봇을 사용자 승인된 다음 locale의 표준 난이도 fresh S1부터 실행한다.
...
8. full-run cycle이 실패하거나 UI overflow/텍스트 잘림이 발견되면 문서만 바꾸고 재실행하지 않는다.
...
9. full-run cycle 도중 세션이 종료되면 다음 세션에서는 마지막 output-dir, 10_contest_full_run_bot.log, checkpoint 상태를 먼저 확인하고 이어간다.
...
- 보스 클리어 슬롯 해금 보상은 S2 Quick Item, S4 Passive, S6 Jester 순서다. 다음 Market 진입에서만 1회 해금 연출을 보여주고 전투에는 해금 상태로 들어가야 한다.
- 정산 sheet는 PhoneFrame 안에 있어야 한다. desktop/web route overlay에서 전체 화면 폭으로 그려지면 실패다.
- 정산 sheet 텍스트에 노란 밑줄이 보이면 `TextDecoration` 상속 회귀로 보고 제출 QA 결함으로 처리한다.
