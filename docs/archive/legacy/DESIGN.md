...
- 하단 4분할
- `확정`이 가장 중요하지만, 현재 구조상 네 버튼 모두 동등한 폭을 유지
- 이후 리디자인에서 `확정`만 더 밝거나 발광도를 높여 우선순위를 줄 수 있음
- Stitch 목업에서는 버튼 텍스트를 아래 중 하나로 통일한다
  - 한국어: `보드 버림`, `손패 버림`, `확정`, `선택 해제`
...
- `버림`, `확정`, `선택 해제`는 한 줄에 두고 엄지 영역에서 바로 누를 수 있어야 한다
...
- 현재 타일 외형 기준은 [`lib/game/rummi_poker_grid/rummikub_tile_canvas.dart`](../../../lib/game/rummi_poker_grid/rummikub_tile_canvas.dart) 이다.
- 현재 레이아웃 구조 기준은 [`lib/views/game_view.dart`](../../../lib/views/game_view.dart) 이다. 과거 `rummi_poker_grid_game.dart` Flame 경로는 삭제된 레거시 경로다.
