...
| UI-2 | 선택된 보드 타일이 있을 때 `이동` 버튼 추가 |
| UI-3 | `_boardMoveMode` / `_pendingBoardMoveSource` UI state 추가 |
| UI-4 | 이동 모드 전용 barrier + 보드 레이어 추가 |
| UI-5 | 이동 가능한 빈 칸 / 잠긴 타일 칸 / 선택 원본 칸 visual state 추가 |
| UI-6 | 빈 칸 선택 시 이동 확인 dialog 추가 |
| UI-7 | 팝업 취소 시 이동 모드 유지, 원본 타일 재터치 시 이동 모드 해제 |
| UI-8 | 영향 라인 highlight / 위험 문구 추가 |
| UI-9 | item/Jester 이동 효과 toast/badge 연결 |
