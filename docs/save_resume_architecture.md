...
- 게임오버는 현재 **`다시하기 / 종료` 팝업**으로 처리한다.
  - `다시하기`: `stageStartSession` / `stageStartRunProgress` 기준으로 즉시 복원
  - `종료`: 저장을 삭제하고 타이틀로 이동
- 따라서 게임오버 직전 저장은 “죽은 상태 그대로 이어하기”가 아니라, **현재 스테이지 시작 상태를 다시 시도할 수 있는 저장**으로 보는 편이 맞다.
