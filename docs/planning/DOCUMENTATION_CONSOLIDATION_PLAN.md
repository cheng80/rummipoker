...
3. archive 이동 후보 보류/분류
   - 오래된 계획
   - 현재 정책과 충돌하는 실험안
   - 완료된 임시 QA 기록
   - 실제 ML이 아닌데 ML 적용처럼 읽히는 과거 보고서
   - 단, `START_HERE.md` 또는 current 문서에서 아직 읽는 문서는 이동하지 않는다.
...
| `docs/planning/STATUS.md` | `START_HERE.md`가 읽던 작업 진행 결과물/기존 진행 snapshot | 이동 보류. 필요한 최신 항목을 current 문서로 승격하고 `START_HERE.md` 경로를 맞춘 뒤 판단 |
| `docs/planning/IMPLEMENTATION_PLAN.md` | 과거 대형 실행 계획 | 이동 보류. 남은 미완료 항목을 `OVERALL_GOAL_PROGRESS.md`로 승격한 뒤 판단 |
| `docs/planning/MIGRATION_ROADMAP.md` | 과거 migration roadmap | 이동 보류. 현재 migration blocker가 남아 있는지 확인 후 판단 |
...
아래 문서는 바로 삭제하거나 이동하지 않는다. 먼저 `START_HERE.md`와 current 문서의 참조를 확인하고, 필요한 내용을 current source-of-truth로 승격한 뒤 `docs/archive/` 하위로 이동하거나 상단에 snapshot 표시를 더한다.

| 후보 | 이유 | 선행 확인 |
|---|---|---|
| `docs/planning/STATUS.md` | `START_HERE.md`가 읽던 작업 진행 결과물이라 이동 순서를 잘못 잡으면 진입 경로가 깨짐 | 필요한 최신 항목 승격, `START_HERE.md` 경로 수정, 링크 참조 확인 |
