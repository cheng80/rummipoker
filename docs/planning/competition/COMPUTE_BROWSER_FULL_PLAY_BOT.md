...
2026-05-09 현재 `contest_full_run_bot`은 최신 룰/UI 후보에서 fresh 표준 난이도 S1~S8 boss pass 증거를 확보했다. 2026-05-10에는 `ko` locale 표준→도전 1사이클도 통과했다.
이전 체크포인트/재시도 기반 S8 boss pass 증거와, S8 boss 실패/timeout을 만들었던 최신 보정 후보 로그도 기준선으로 남긴다.
다음 full-run 재개는 사용자 승인 후 `en` fresh 표준 gate다. 제출 gate는 지원 locale 5개(`ko`, `en`, `ja`, `zh-CN`, `zh-TW`) 각각에서 fresh 표준 난이도 S1~S8 Boss를 먼저 클리어한 뒤, 같은 locale fresh 도전 난이도 S1~S8 Boss와 S8 정산/보상/무한 도전 진입 직전까지 확인해 닫는다. 5개 locale을 한 번에 연속 실행하지 않고 한 locale cycle 완료/점검/사용자 승인 후 다음 locale을 시작한다.
...
- 종료 후 WebDriver Chrome, Chrome Helper, ChromeDriver, Flutter web 서버 잔류 프로세스 없음
- 다음 상태:
  - `ko` cycle은 닫혔다. 사용자 승인 후 `en` fresh 표준 난이도 S1부터 실행한다.
...
- 최신 fresh 표준 run에서는 족보 레벨 성장과 덱 확장/보상 타일 축이 들어간 상태로 S8 boss까지 통과했다. `ko` 도전 난이도도 S8 boss까지 통과했으며, 남은 locale의 도전 난이도는 아직 미검증이다.
...
- locale별 표준→도전 cycle 가능성: `ko` 1/5 cycle 통과. `en`, `ja`, `zh-CN`, `zh-TW` fresh cycle은 미검증이며 다음 full-run gate다.
