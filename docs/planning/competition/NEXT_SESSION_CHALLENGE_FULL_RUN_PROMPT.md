...
7. full-run cycle이 실패하거나 UI overflow/텍스트 잘림이 발견되면 문서만 바꾸고 재실행하지 않는다.
...
8. full-run cycle 도중 세션이 종료되면 다음 세션에서는 마지막 output-dir, 10_contest_full_run_bot.log, checkpoint 상태를 먼저 확인하고 이어간다.
