...
| catalog audit v2/v3 price probe | Workspace pending | `tools/sim/run_balance_sim.dart` / `tools/sim/economy_audit.py` | `catalog_audit_v2`는 `trade_ticket`/`ride_the_bus` floor, `catalog_audit_v3`는 고급 study 완화 가설을 sim-only로 추가. same-seed r120에서 v3와 normalized가 동일해 현재 sim path는 해당 실제 카탈로그 후보 구매 이벤트를 검증하지 못한다. |
...
- 2026-06-09 policy correction: 제품 UI/런타임 기준 첫 리롤 무료는 모든 Market entry bonus가 아니라 S1 기본 첫 Market(`stageIndex == 1 && currentStationBlindTierIndex == 0`) 이용 보상 1회다. 같은 Market 안에서 한 lane이 사용하면 다른 lane에는 남지 않고, 이후 Market이나 S2+에서는 다시 생기면 안 된다.
- restore/display guard: effective cost 계산과 legacy save restore에서도 eligibility를 다시 확인한다. 오래된 저장에 `firstRerollDiscount`가 남아 있어도 S1 첫 Market이 아니면 `리롤 5`로 보이고, S1 첫 Market 보상은 `리롤 5→0`이 아니라 `첫 리롤 무료`로 표시한다.
