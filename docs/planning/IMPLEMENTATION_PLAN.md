...
- Economy: start gold 0, stage clear base 10, reroll 5
...
- Starting deck/Jester/Consumable/Voucher reference taxonomy도 archive에 보관한다. 현재 New Run은 Random/Seed만 유지하고, 후속 덱 선택은 `run_archetype_id`, 타일 강화는 `tile_modifier_id` 기준으로 ML/simulator에 먼저 연결한다.
...
- `New Run`은 전용 route로 분리됐고, 현재 가능한 `Random / Input Seed` entry만 제품 화면에 노출한다. future setup/덱 placeholder는 일반 화면에서 제거했다. 후속 starting deck 선택은 `run_archetype_id` 기준으로 simulator/log 경계가 잡힌 뒤 검토한다.
