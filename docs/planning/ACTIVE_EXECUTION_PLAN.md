...
현재는 공모전 기준 QA를 재개한다. 다음 남은 gate는 도전 난이도 fresh S1~S8 Boss `contest_full_run_bot` full-run을 지원 locale 5개(`ko`, `en`, `ja`, `zh-CN`, `zh-TW`) 각각 1회씩 실행하는 것이다. 플레이 범위는 S8 Boss 클리어와 S8 정산/보상/무한 도전 진입 직전 확인까지이며, S9+ 무한 도전 장기 생존은 별도 확장 검증이다.
...
1. `contest_full_run_bot`을 도전 난이도 fresh S1부터 최신 build 기준으로 실행한다. locale 순서는 `ko` -> `en` -> `ja` -> `zh-CN` -> `zh-TW`다.
