...
3. 완료: 9개 후보는 너무 적다는 판단으로 폐기하고, 새 카드 후보 pool을 40종으로 확장했다. 이후 기억 의식 3종은 재설계 가치가 낮아 삭제했다.
4. 완료: 과거에는 첫 draft 18종/구현 slice 8종을 먼저 권장했지만, 현재는 Ritual/Item 확장 계열 37종을 catalog/runtime에 둔 상태다. 이후 판단은 `ITEM_POLICY_CLEANUP_AUDIT.md`의 V1 QA/정책 정리 항목을 따른다.
...
13. ML/시뮬레이션/풀런봇 계약은 운명 카드 QA와 pool/가격 1차 정리 뒤 붙인다.
...
- `docs/release/submission_kit/` 제출 문서 세트 정리 완료. 이번 웹 제출 기준에서는 문서화로 닫고, Android/iOS 실제 release artifact 생성은 해당 플랫폼 제출 시 별도 gate로 둔다.
