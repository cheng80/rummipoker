...
| Roguelite meta | 게임오버 이후 보상으로 다음 run 선택지가 열린다 | In progress | 14% | Insight 보상 표시 이후 최소 해금 루프 UX 정리 |
| Run restart loop | 패배/클리어 후 보상, 해금, 새 run 시작이 자연스럽게 이어진다 | In progress | 14% | 게임오버 보상 표시 이후 title/new run 연결 QA |
...
| Roguelite meta growth | 14% | Insight, high stakes 해금, 게임오버 보상 표시 기반 존재 | unlock tree 확장과 새 run 연결 QA |
| Game over reward loop | 18% | RunProgressionService 보상 산식, Insight 저장, 게임오버 보상 UI 표시 | 클리어/패배 결과 화면 통합 polish |
...
| Roguelite loop stub | 게임오버 보상과 다음 run 복귀가 최소 형태로 존재한다 | In progress | 22% |
