...
처음 보는 사람은 용어를 모두 외울 필요가 없다. 먼저 `GAME_DESIGN`에서 게임 흐름을 읽고, 필요한 규칙만 `GAME_RULES`와 `RUN_ECONOMY`에서 찾아본다. `runtime`은 실제 실행 상태, `restore`는 저장한 게임을 다시 여는 일이라고 이해하면 된다.
...
- 화면·튜토리얼·연출·사운드: [UI_UX](docs/core/UI_UX.md)
- 번역 키·줄바꿈·다국어 작업 절차: [I18N](docs/core/I18N.md)
