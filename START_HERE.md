...
- 핵심 룰은 현재 문서 기준으로 고정되어 있다.
  ...
  - **5칸 완성 강제는 제거**되었고, 부분 줄도 현재 놓인 카드만으로 즉시 평가한다
  - **하이카드와 원페어**는 점수 없는 줄이며, **투페어 이상부터만 족보 점수**로 취급한다
  - 확정 시 **겹침 기여(overlap)** 가 있으면 라인 기본 점수에 배수가 붙는다
...
- 최근 작업:
  ...
  - 액션 버튼은 이제 **보드 버림 / 손패 버림 / 확정 / 선택 해제**로 분리되어, 보드 타일 버림과 손패 버림을 혼동하지 않게 정리했다.
  ...
  - 게임오버는 이제 **`다시하기 / 종료` 팝업**으로 처리한다.
    - `다시하기`: 현재 스테이지 시작 시점(`stageStartSnapshot`)으로 즉시 복원
    - `종료`: 기존처럼 저장을 지우고 타이틀로 이동
  - 디버그용 재현 상태는 `DebugRunFixtureService`에 등록하고, 타이틀의 **디버그 픽스처 선택 UI**로 바로 플레이할 수 있다.
...
**현재 1순위는 “즉시 확정/부분 줄 평가/overlap/이어하기 저장 기준이 코드와 문서에서 다시 어긋나지 않게 유지하면서, 남은 기능 작업(재시작 검증, rule_modifier 분류, 검사 상점 정리, 유저 문구 정리)을 이어가는 것”이다.**
...
- 빈 칸이 있는 줄도 **현재 놓인 카드만으로 판정**
- 카드 수별 성립 족보: **2장 원페어 / 3장 트리플 / 4장 투페어·포카드 / 5장 전체 족보**
- Straight: 일반 연속 + **10–11–12–13–1**
- 죽은 줄: **하이카드 + 원페어**
- 일괄 확정: 현재 점수 성립 가능한 줄 **전부**를 한 번에 점수화/제거
- overlap: 기여 타일이 여러 줄에 걸치면 **최고 `k` 기준 배수**를 라인 점수에 적용
- 조커: **보드 타일 아님**, 메타 슬롯만 사용
- 덱: `copiesPerTile` 기반, 현재 기본은 `1`
- 만료: `D_board == 0` 상태 보드 25칸 만재 또는 현재 덱 전부 소모

## 6. 현재 메모

- 사용자 확정 규칙은 **즉시 확정 + 부분 줄 평가 + overlap 보너스** 방향으로 갱신됐다.
- 버튼 라벨은 이제 **`줄 확정`이 아니라 `확정`** 이다.
...
- 게임오버는 **`다시하기 / 종료` 팝업**으로 분기한다.
  - `다시하기`: 현재 스테이지 시작 스냅샷 복원
  - `종료`: 세이브 삭제 후 타이틀 이동
...
- 현재 남은 큰 묶음은 아래 두 가지다.
  - `rule_modifier`
  - `retrigger`
- 디버그 재현용 픽스처는 `lib/services/debug_run_fixture_service.dart`에 등록하고, 타이틀의 디버그 메뉴에서 선택해 실행한다.
