...
int _simPriceBandCostForEvent({
  ...
  if (mode == BalanceSimPriceBandMode.catalogAuditV2) {
...
int _simCatalogAuditV2CostForEvent({
  required Map<String, Object?> event,
  required int fallbackCost,
}) {
  final category = event['category'];
  ...
  if (category == 'item' && contentId is String) {
    return switch (contentId) {
      ...
      'coin_cache' => max(fallbackCost, 4),
      'thin_wallet' => max(fallbackCost, 7),
      'trade_ticket' => max(fallbackCost, 8),
      _ => fallbackCost,
    };
  }
  if (category != 'jester') return fallbackCost;
  ...
  final ids = proxyIds is List
      ? proxyIds.whereType<String>().toSet()
      : <String>{if (contentId is String) contentId};
  var cost = _simCatalogNormalizedCostForEvent(
    event: event,
    fallbackCost: fallbackCost,
  );
...
({int scorePenalty, int triggerCount}) simBossConstraintPenalty({
  ...
  for (final line in lineBreakdowns) {
    ...
    final rankFamily = _simBossRankFamily(line.rank);
    ...
    if (constraint.rankFamilyDecayScoreMultiplier case final multiplier?
        when usedRanks.map(_simBossRankFamilyName).contains(rankFamily)) {
      penalties.add(1 - multiplier);
    }
    ...
    if (constraint.minContributingTileCount case final minimum?
        when _simBossContributorCount(line) < minimum) {
      penalties.add(1 - constraint.minContributorScoreMultiplier);
    }
...
enum _BalanceSimRankFamily { set, sequence, color, hybrid, dead }

_BalanceSimRankFamily _simBossRankFamily(RummiHandRank rank) {
  return switch (rank) {
...
_BalanceSimRankFamily _simBossRankFamilyName(String rankName) {
  final rank = RummiHandRank.values.byName(rankName);
  return _simBossRankFamily(rank);
}

int _simBossContributorCount(ConfirmedLineBreakdown line) {
  // 과거 summary/test용 breakdown에는 기여 칸이 비어 있을 수 있어
  // 실제 5칸 라인으로 간주하고 simulation-only penalty 오판을 막는다.
  return line.contributingCells.isEmpty ? 5 : line.contributingCells.length;
}
...
BalanceSimExperimentSpec _resolveExperiment({
  ...
  switch (id) {
    ...
    case _
        when id.contains('_boss_expansion_stage_a_target_spike_probe_v1') ||
...
bool _usesRankCycleProbe(String id) =>
    ...
    _usesStageABossExpansionSplitProbe(id);
...
String _rankCycleProbeProfileId(String id) {
  ...
  if (id.contains('_rank_runtime_position_probe_v1_repeat_s4')) {
    return 'rank_runtime_position_probe_v1_repeat_s4';
  }
  if (id.contains('_rank_runtime_position_probe_v1_repeat_s5')) {
    return 'rank_runtime_position_probe_v1_repeat_s5';
  }
  if (id.contains('_rank_runtime_position_probe_v1_repeat_s6')) {
    return 'rank_runtime_position_probe_v1_repeat_s6';
  }
  if (id.contains('_rank_runtime_position_probe_v1_single_s4')) {
    return 'rank_runtime_position_probe_v1_single_s4';
  }
  if (id.contains('_rank_runtime_position_probe_v1_single_s5')) {
    return 'rank_runtime_position_probe_v1_single_s5';
  }
  ...
  if (id.endsWith('_boss_expansion_min_contributor_probe_v1')) {
    return 'boss_expansion_min_contributor_probe_v1';
  }
  if (id.endsWith('_boss_expansion_rank_family_probe_v1')) {
    return 'boss_expansion_rank_family_probe_v1';
  }
  ...
  if (id.endsWith('_boss_expansion_stage_a_target_spike_probe_v1')) {
    return 'boss_expansion_stage_a_target_spike_probe_v1';
  }
  ...
  if (id.endsWith('_boss_expansion_stage_a_reward_tax_probe_v1')) {
    return 'boss_expansion_stage_a_reward_tax_probe_v1';
  }
  if (id.endsWith('_boss_expansion_stage_a_color_variant_probe_v1')) {
    return 'boss_expansion_stage_a_color_variant_probe_v1';
  }
...
List<int> _rankCycleProbeSlotsForExperiment(String id) {
  ...
  if (id.contains('_rank_runtime_position_probe_v1_repeat_s5')) {
    return const [0, 1, 2, 10, 3, 11, 7, 5];
  }
  if (id.contains('_rank_runtime_position_probe_v1_repeat_s6')) {
    return const [0, 1, 2, 10, 6, 3, 7, 5];
  }
  if (id.contains('_rank_runtime_position_probe_v1_single_s4')) {
    return const [0, 1, 2, 4, 6, 11, 7, 5];
  }
  if (id.contains('_rank_runtime_position_probe_v1_single_s5')) {
    return const [0, 1, 2, 10, 4, 11, 7, 5];
  }
  if (id.contains('_rank_runtime_position_probe_v1_single_s6')) {
    return const [0, 1, 2, 10, 6, 4, 7, 5];
  }
  ...
  if (id.endsWith('_boss_expansion_min_contributor_probe_v1')) {
    return const [0, 14, 2, 10, 6, 11, 7, 5];
  }
  if (id.endsWith('_boss_expansion_rank_family_probe_v1')) {
    return const [0, 1, 2, 15, 6, 11, 7, 5];
  }
  if (id.endsWith('_boss_expansion_confirm_limit_probe_v1')) {
    return const [0, 1, 2, 10, 6, 16, 7, 5];
  }
  ...
  if (id.endsWith('_boss_expansion_stage_a_target_spike_probe_v1')) {
    return const [17, 1, 2, 10, 6, 11, 7, 5];
  }
  if (id.endsWith('_boss_expansion_stage_a_hand_discard_probe_v1')) {
  ...
  if (id.endsWith('_boss_expansion_stage_a_reward_tax_probe_v1')) {
    return const [0, 1, 2, 20, 6, 11, 7, 5];
  }
  if (id.endsWith('_boss_expansion_stage_a_color_variant_probe_v1')) {
    return const [0, 1, 2, 10, 21, 11, 7, 5];
  }
...
BalanceSimBossConstraintChoice _bossConstraintChoiceForSlot({
  ...
  return switch (slot) {
    ...
    14 => choice(
      ...
      sourceReference: 'Psychic-style minimum contributing tile pressure',
      simConstraint: const BalanceSimBossConstraint(
        id: 'min_contributor_count_v1',
        family: 'line_contributor_requirement',
        sourceReference: 'Psychic-style minimum contributing tile pressure',
    ...
    15 => choice(
      ...
      sourceReference: 'Arm-style rank family decay pressure',
      simConstraint: const BalanceSimBossConstraint(
        id: 'rank_family_decay_v1',
        family: 'rank_family_decay',
        sourceReference: 'Arm-style rank family decay pressure',
    ...
    16 => choice(
      ...
      family: 'confirm_limit_tax',
      sourceReference: 'Needle-style late confirm tax pressure',
      targetScoreMultiplier: 0.82,
      simConstraint: const BalanceSimBossConstraint(
        id: 'confirm_limit_tax_v1',
        family: 'confirm_limit_tax',
        sourceReference: 'Needle-style late confirm tax pressure',
        confirmAfterLimitActionCount: 1,
...
class BalanceSimBossConstraint {
  ...
  Map<String, Object?> toJson() {
    return <String, Object?>{
      ...
      if (rankFamilyDecayScoreMultiplier != null)
        'rank_family_decay_score_multiplier': rankFamilyDecayScoreMultiplier,
...
class BalanceSimCliConfig {
  ...
  static String _parseExperimentId(String raw) {
    const ids = {
      ...
      'base_score_curve_v2_boss_constraint_pool_v4_s1_soft_v2_late_guard_v1_s1_resource_weighted_boss_v3_late_boss_068_rank_cycle_single_only_probe_v1',
      'base_score_curve_v2_boss_constraint_pool_v4_s1_soft_v2_late_guard_v1_s1_resource_weighted_boss_v3_late_boss_068_rank_cycle_soft_probe_v1',
      'base_score_curve_v2_boss_constraint_pool_v4_s1_soft_v2_late_guard_v1_s1_resource_weighted_boss_v3_late_boss_068_rank_runtime_position_probe_v1_repeat_s4',
