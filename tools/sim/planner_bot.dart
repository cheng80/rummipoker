...
class PlannerBotPolicy extends BalanceSimBotPolicy {
    ...
    for (var handIndex = 0; handIndex < session.hand.length; handIndex++) {
      ...
      for (var row = 0; row < kBoardSize; row++) {
        for (var col = 0; col < kBoardSize; col++) {
          ...
          final potentialScore = _boardPotentialScore(copy.board);
  ...
  int _boardPotentialScore(RummiBoard board) {
    var score = 0;
    for (var row = 0; row < kBoardSize; row++) {
      score += _linePotentialScore(board.row(row));
    }
    for (var col = 0; col < kBoardSize; col++) {
      score += _linePotentialScore(board.col(col));
    }
    score += _linePotentialScore(board.diagMain());
    score += _linePotentialScore(board.diagAnti());
    return score;
  }

  int _linePotentialScore(List<Tile?> line) {
    ...
    final straightRun = _longestStraightRun(rankCounts.keys.toList());
  ...
  int _longestStraightRun(List<int> ranks) {
  ...
  BalanceSimAction? _conservativeDiscard(RummiPokerGridSession session) {
    ...
    (int, int)? worstCell;
    var worstLoss = 1 << 30;
    final basePotential = _boardPotentialScore(session.board);
    for (var row = 0; row < kBoardSize; row++) {
      for (var col = 0; col < kBoardSize; col++) {
        if (session.board.cellAt(row, col) == null) continue;
        final copy = session.board.copy();
        copy.setCell(row, col, null);
        final loss = basePotential - _boardPotentialScore(copy);
        if (loss < worstLoss) {
          worstLoss = loss;
          worstCell = (row, col);
        }
