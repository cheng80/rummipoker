  ...
  for (var runIndex = 0; runIndex < config.runs; runIndex++) {
    ...
    for (var turn = 0; turn < config.turnCap; turn++) {
      ...
      final request = buildLimitedLlmSmokeRequest(
        ...
        index: runIndex,
        turnCount: turn,
        maxLegalActions: config.maxLegalActions,
      );
      final responseResult = await _requestLlmAction(
        request: request,
        config: config,
      );
...
Future<({Map<String, dynamic>? response, int latencyMs, String? error})>
_requestLlmAction({
  required LlmActionRequest request,
  required _Config config,
}) async {
  final safeId = request.requestId.replaceAll(RegExp(r'[^A-Za-z0-9_.-]'), '_');
  final requestPath = 'logs/llm/turn_loop_requests/$safeId.json';
  final responsePath = 'logs/llm/turn_loop_responses/$safeId.json';
  ...
  try {
    ...
    if (response is Map<String, dynamic>) {
      return (response: response, latencyMs: latencyMs, error: null);
    }
    return (response: null, latencyMs: latencyMs, error: 'non_object_response');
  } on TimeoutException {
    return (
      response: null,
...
Map<int, Map<String, Object?>> _lastRowByRun(
