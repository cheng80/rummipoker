...
- 결론: 현재 classifier는 feature sanity check 수준이며 추천 gate 완료 근거가 아니다.
- 핵심 점수: Accuracy 0.8395, balanced accuracy 0.6649, F1 0.9065.
- 데이터: 323 rows, train 242, test 81, target `cleared_majority`.
- task: `classification`.
- 사용 가능: class 분포와 feature sanity check.
...
- 다음 액션: candidate grid와 class balance를 늘리고 balanced accuracy/F1을 재평가한다.

## 핵심 점수

| 항목 | 현재값 | 기준 | 판단 |
|---|---:|---|---|
| Accuracy | 0.8395 | 높을수록 좋음 | 참고 지표 |
| Balanced accuracy | 0.6649 | class imbalance에서도 높아야 함 | 실무 추천 기준에는 부족 |
| F1 | 0.9065 | positive class 재현 필요 | 기준 정의 필요 |
| ROC-AUC | 0.7319 | 높을수록 좋음 | 참고 지표 |
| Row | 323 | 많을수록 좋음 | 데이터 규모 확인용 |
...
| Feature | 중요도 |
|---|---:|
| `station_pressure_interaction` | 0.8182 |
| `station_tier_index` | 0.0725 |
| `resolved_market_profile_s1_candidate_tarot_build_pack` | 0.0273 |
| `market_station_interaction` | 0.0134 |
| `station_boss_interaction` | 0.0100 |
| `market_availability_index` | 0.0068 |
| `market_profile_version` | 0.0062 |
| `station` | 0.0051 |
| `resolved_market_profile_s1_candidate_common_rank_jester` | 0.0049 |
| `sim_boss_constraint_id_blue_dampener_v1` | 0.0048 |

## 산출물

- metrics JSON: `analysis/leveling/models/fresh_contest_policy_grid_20260529_preoutcome/cleared_majority/cleared_majority_preoutcome_metrics.json`
- feature importance CSV: `analysis/leveling/models/fresh_contest_policy_grid_20260529_preoutcome/cleared_majority/cleared_majority_preoutcome_feature_importance.csv`
