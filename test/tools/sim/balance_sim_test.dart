...
void main() {
  ...
  test(
    'CLI sim catalog value flag price band raises flagged proxy costs',
    () async {
      final dir = Directory.systemTemp.createTempSync('balance_sim_test_');
      addTearDown(() => dir.deleteSync(recursive: true));

      final outPath = '${dir.path}/catalog_value_flags.jsonl';
      final code = await runBalanceSim([
        '--runs',
        '1',
        '--bot',
  ...
  test(
    'CLI sim catalog audit v3 price band is accepted in economy trace',
    () async {
      final dir = Directory.systemTemp.createTempSync('balance_sim_test_');
      addTearDown(() => dir.deleteSync(recursive: true));

      final outPath = '${dir.path}/catalog_audit_v3.jsonl';
      final code = await runBalanceSim([
        '--runs',
        '1',
        '--bot',
      ...
      final bigRow = battleRows.firstWhere((row) => row['blind_tier'] == 'big');
      final trace = bigRow['sim_economy_trace'] as Map<String, dynamic>;
      expect(trace['price_band_mode'], 'catalog_audit_v3');
      expect(trace['known_market_spend'], isA<int>());
    },
  );
  ...
  test(
    'CLI rank cycle probe places rank bosses without runtime mutation',
    () async {
      final dir = Directory.systemTemp.createTempSync('balance_sim_test_');
      addTearDown(() => dir.deleteSync(recursive: true));

      final outPath = '${dir.path}/rank_cycle_probe.jsonl';
      final code = await runBalanceSim([
        '--runs',
        '1',
        '--bot',
      ...
      for (final row in rows) {
        final effects = row['experiment_effects'] as Map<String, dynamic>;
        expect(effects['rank_cycle_probe'], isTrue);
        expect(effects['ordered_boss_targets'], isTrue);
      }
    },
  );
