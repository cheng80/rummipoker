...
void main() {
  ...
  testWidgets('discounted reroll shows original and discounted prices', (
    tester,
  ) async {
    ...
    final jester = const RummiJesterCard(
      id: 'jester_offer',
    ...
    final quickSlotItem = ItemDefinition.fromJson(const <String, dynamic>{
      'id': 'board_scrap',
    ...
    expect(find.text('리롤 5→4'), findsOneWidget);

    await tester.tap(find.text('리롤 5→4'));
    await tester.pumpAndSettle();

    expect(find.text('Jester 후보를 리롤할까요?\n리롤 할인 적용: 5G → 4G'), findsOneWidget);
    expect(find.text('리롤'), findsOneWidget);
  });
}
